...
contract WETHConverter {
    ...
    function convertwethAaveTowethCom(uint256 amount) external {
        _convert(wethAave, wethCom, amount);
    }
    
    function convertwethComTowethAave(uint256 amount) external {
    ...
    function _convert(IWETH fromWETH, IWETH toWETH, uint256 amount) internal {
        ...
        emit WETHConverted(msg.sender, address(fromWETH), address(toWETH), amount);
    }
    ...
    // Required to receive ETH from WETH withdraw
